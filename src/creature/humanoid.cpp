                   ...
                   float sizeYMeters) : BaseCreature(registry, self, worldId, position, PhysicsUtils::getNextNegativeId())
{
    ...
    // Set groupId of all body parts to groupId
    // TODO: maybe find a better way to set index upon creating bodies as this way is costly?
    for (auto ent : this->bodies)
    {
        PhysicsBody body = this->registry.get<PhysicsBody>(ent);
        for (auto shId : body.shapes)
        {
            b2Filter f = b2Shape_GetFilter(shId);
            f.groupIndex = this->groupId;
            b2Shape_SetFilter(shId, f);
        }
    }
}
