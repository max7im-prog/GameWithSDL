...
void DebugRenderer::visit(Circle &c) {
  ...
  RenderUtils::renderCircle(
      center, radius, getSDLColor(RenderColor::CIRCLE_COLOR), _renderContext);
}
...
void DebugRenderer::visit(Polygon &p) {
  ...
  RenderUtils::renderPolygon(
      screenPoints, getSDLColor(RenderColor::POLYGON_COLOR), _renderContext);
}
...
void DebugRenderer::visit(Capsule &c) {
  ...
  auto radius = c.getRadius() * _renderContext.getPixelToMeterRatio();
  RenderUtils::renderCapsule(
      p1, p2, radius, getSDLColor(RenderColor::CAPSULE_COLOR), _renderContext);
}
